...
def remove_cart_item(cursor, item_id, user_account):
    cursor.execute(
        f'''DELETE FROM `{settings.BRANCH_C_CART_TABLE}`
            WHERE id = ?
...
def clear_cart(cursor, user_account):
    cursor.execute(
        f'''DELETE FROM `{settings.BRANCH_C_CART_TABLE}`
            WHERE user_id = (SELECT id FROM `{settings.BRANCH_A_TABLE}` WHERE user_account = ?)''',
        (user_account,)
    )
...
def add_product(cursor, name, price, description, img_filename):
    cursor.execute(f"""
        INSERT INTO `{settings.BRANCH_B_TABLE}`
...
def set_product_active(cursor, product_id, is_active):
    cursor.execute(f"""
        UPDATE `{settings.BRANCH_B_TABLE}`
        SET `is_active` = ?
        WHERE `id` = ?
...
def get_all_products(cursor):
    cursor.execute(f"""
        ...
        FROM `{settings.BRANCH_B_TABLE}` p
